...
const PlanetsCategory = () => {
    ...
    const fetchPlanets = async() => {
        const planetsResponse = await axios.get("https://swapi.py4e.com/api/planets/")
            .catch((error) => {
                console.log(error)
            })
    ...
    return(
        ...
        <div className='container' style={{margin: '50px 30px', }}>
           ...
           All Planets!
            ...
            <div className='grid'>
                {
                    currentPlanets && currentPlanets.map(planet => {
                        const { created, name, url, id } = planet;
                            return(
                                <div className="card" key={created} style={{borderColor: 'red'}}>
            ...
            <Pagination postsPerPage={planetsPerPage} totalPosts={planets.length} paginate={paginate} />
        </div>
    </>
