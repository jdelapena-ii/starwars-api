...
const PeopleCategory = () => {
    ...
    const fetchPlanets = async() => {
        const peopleResponse = await axios.get("https://swapi.py4e.com/api/people/")
            .catch((error) => {
                console.log(error)
            })
    ...
    return(
        ...
        <div className='container' style={{margin: '50px 30px', }}>
           ...
           All People Baby!
            ...
            <div className='grid'>
                {
                    currentPeople && currentPeople.map(person => {
                        const { created, name, url, id } = person;
                            return(
                                <div className="card" key={created} style={{borderColor: 'green'}}>
            ...
            <Pagination postsPerPage={peoplePerPage} totalPosts={people.length} paginate={paginate} />
        </div>
    </>
